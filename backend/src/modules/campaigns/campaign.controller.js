...
// ─── Create Campaign ──────────────────────────────────────────────────────────
// Uses upload.fields in route — req.files.image[0] + req.files.documents[]
export const createCampaign = async (req, res) => {
  try {
    const campaign = await createCampaignService({
      body: req.body,
      files: req.files, // ← changed from req.file to req.files
      userId: req.user._id,
    });
...
export const getCampaignById = async (req, res) => {
  try {
    const campaign = await getCampaignByIdService(req.params.id, req.user?.role);
